...
class VehicleViewModel @Inject constructor(
    ...
    val editText = savedStateHandle.getStateFlow("editText", "")

    //User Handled Events
    private var savedRecord = Record()
    fun getRecord(): Record = savedRecord
    fun setRecord(record: Record) { savedRecord = record }

    fun onChangeState(searchText: String = "", editText: String = "") {
        if (searchText.isNotEmpty()) savedStateHandle["searchText"] = searchText
        if (editText.isNotEmpty()) savedStateHandle["editText"] = editText
    }

    //Easier way to display vehicle details in Details Screen
    ...
    fun getLocation(vehicleId: String, locationId: String) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            savedStateHandle["location"] =
                listOf(response.geolocation?.latitude, response.geolocation?.longitude)
        }
